...
This project is uses an ESP32-S3 to commmunicate with the Castle ESC to controll the motors. It communicates with the ESC using UART and reads various parameters such as voltage, current, throttle, and RPM. It also allows setting the throttle value.
...
1. Clone the repository:
    ```sh
    git clone https://github.com/yourusername/TobyMotorPIDLoop.git
    ```
...
3. You can send throttle values (between 1.0 and 2.0 ms) via the serial monitor to control the ESC.

## To-Do List

- [ ] Use escape codes for better serial logging
